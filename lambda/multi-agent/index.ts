...
import { MultiAgentOrchestrator, BedrockLLMAgent, DynamoDbChatStorage, LexBotAgent, AmazonKnowledgeBasesRetriever, LambdaAgent, BedrockClassifier } from "multi-agent-orchestrator";
...
declare global {
  namespace awslambda {
    function streamifyResponse(f: (event: APIGatewayProxyEventV2, responseStream: NodeJS.WritableStream, context: Context) => Promise<void>): Handler;
  }
}
...
const storage = new DynamoDbChatStorage(process.env.HISTORY_TABLE_NAME!, process.env.AWS_REGION!, process.env.HISTORY_TABLE_TTL_KEY_NAME, Number(process.env.HISTORY_TABLE_TTL_DURATION));
...
const maoDocAgent = new BedrockLLMAgent({
  ...
  retriever: new AmazonKnowledgeBasesRetriever(new BedrockAgentRuntimeClient(), {
...
async function eventHandler(event: APIGatewayProxyEventV2, responseStream: NodeJS.WritableStream) {
  // logger.info(event);
  ...
  try {
    ...
    const response = await orchestrator.routeRequest(userBody.query, userId, sessionId);
    ...
    if (response.streaming == true) {
      ...
      logger.info(`> Additional Parameters:`, response.metadata.additionalParams);
      logger.info(`\n> Response: `);
