...
export class ChatDemoStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const assetsPath = path.join(__dirname, "../../multi-agent-orchestrator/docs/src/content/docs/");
    const assetDoc = s3deploy.Source.asset(assetsPath);

    const assetsTsPath = path.join(__dirname, "../../multi-agent-orchestrator/typescript/src/");
    const assetTsDoc = s3deploy.Source.asset(assetsTsPath);

    const assetsPyPath = path.join(__dirname, "../../multi-agent-orchestrator/python/src/multi_agent_orchestrator/");
    ...
    const powerToolsTypeScriptLayer = lambda.LayerVersion.fromLayerVersionArn(this, "powertools-layer-ts", `arn:aws:lambda:${cdk.Stack.of(this).region}:094274105915:layer:AWSLambdaPowertoolsTypeScriptV2:2`);

    const basicLambdaRole = new iam.Role(this, "BasicLambdaRole", {
      assumedBy: new iam.ServicePrincipal("lambda.amazonaws.com"),
    });

    basicLambdaRole.addManagedPolicy(iam.ManagedPolicy.fromManagedPolicyArn(this, "basicLambdaRoleAWSLambdaBasicExecutionRole", "arn:aws:iam::aws:policy/service-role/AWSLambdaBasicExecutionRole"));
    ...
    const pythonLambda = new lambda.Function(this, "PythonLambda", {
      ...
      code: lambda.Code.fromAsset(path.join(__dirname, "../lambda/find-my-name")),
      memorySize: 128,
      timeout: cdk.Duration.seconds(10),
    });

    const multiAgentLambdaFunction = new nodejs.NodejsFunction(this, "MultiAgentLambda", {
      entry: path.join(__dirname, "../lambda/multi-agent/index.ts"),
    ...
    multiAgentLambdaFunction.addToRolePolicy(
      new iam.PolicyStatement({
        ...
        resources: [`arn:aws:bedrock:${cdk.Aws.REGION}::foundation-model/*`, `arn:${cdk.Aws.PARTITION}:bedrock:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:knowledge-base/${knowledgeBase.knowledgeBase.attrKnowledgeBaseId}`],
      })
    );
    ...
    if (enableLexAgent) {
      multiAgentLambdaFunction.addToRolePolicy(
        new iam.PolicyStatement({
          ...
          resources: [`arn:aws:bedrock:${cdk.Aws.REGION}::foundation-model/*`, `arn:aws:lex:${cdk.Aws.REGION}:${cdk.Aws.ACCOUNT_ID}:bot-alias/${lexAgent!.lexBotId}/${lexAgent!.lexBotAliasId}`],
        })
      );
    }
  }
